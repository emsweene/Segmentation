...
setwd('/dexter/disk1/smart/msmri/Hopkins_MRI/OASIS_ML/Moment_Stuff')
...
load('logistic_model_with_moments.RData')
...
#####################################################################################################################
	...
	OASIS <- as.data.frame(cbind(FLAIR, T1, T2, FLAIR_10, T1_10, T2_10, FLAIR_20,  T1_20, T2_20, FLAIR_FIRST_3,
	FLAIR_SECOND_3, FLAIR_THIRD_3, T1_FIRST_3, T1_SECOND_3, T1_THIRD_3, T2_FIRST_3, T2_SECOND_3, T2_THIRD_3))
...
#####################################################################################################################	
	...
	filename <- paste(patient_id, "logisitc_moments" , sep = "_")
...
#####################################################################################################################	
	...
	filename <- paste(patient_id, "logisitc_moments_healthy" , sep = "_")
	f.write.nifti(prob_map_1, filename ,size="float",nii=TRUE)
...
logistic_voxel_level_prediction <-prediction(PREDICTION_LOGISITIC, GOLD)
logistic_voxel_level_performance<-performance(logistic_voxel_level_prediction, "tpr",  "fpr")
logistic_auc_voxel<-as.numeric(performance(logistic_voxel_level_prediction, measure = "auc")@y.values)

save(logistic_voxel_level_prediction,logistic_voxel_level_performance,logistic_auc_voxel, file='Moment_ROC_Stuff.RData')
