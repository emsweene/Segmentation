...
t2.blur_10 <- temp[8]
t1.blur_10 <- temp[9]
flair.blur_20 <- temp[10]
pd.blur_20 <- temp[11]
t2.blur_20 <- temp[12]
t1.blur_20 <- temp[13]
output.path <- temp[14]

tempdir <- paste(output.path,'temp',sep='/')

##This function unzips and reads a .nii.gz file FROM A NODE##
readniigz <- function(file){
	if (!file.exists(tempdir)) dir.create(tempdir)
	name <- tempfile(pattern = "tempfile", tmpdir = tempdir, fileext = ".nii")
...
#####################################################################################################################
	writeLines("Creating Probability Maps")	
...
#####################################################################################################################
	writeLines("Applying Inverse Logistic Function")
...
##	filename <- c('_prob_map_1')
	name <- c('OASIS_prob_map_1')
	filename <- paste(output.path,name,sep='/')
	f.write.nifti(prob_map, filename ,size="float",nii=TRUE)
	
####################################################################################################################
## Write probability map
#####################################################################################################################
	writeLines("Writing Probability Map")	
